...
export default function AdminEventsTab() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingEvent) {
        await base44.entities.Event.update(editingbase44.entities.Event.id, payload);
  ...
  const handleEdit = (event) => {
    ...
    setFormData({...event, start_date: base44.entities.Event.start_date ? base44.entities.Event.start_date.slice(0, 16) : '', end_date: base44.entities.Event.end_date ? base44.entities.Event.end_date.slice(0, 16) : ''});
    setShowForm(true);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4"><h4 className="text-lg font-medium text-white flex items-center gap-2"><CalendarDays className="w-5 h-5 text-purple-400" />Próximos Eventos ({upcomingEvents.length})</h4><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{upcomingEvents.map(event => { const typeConfig = getEventTypeConfig(base44.entities.Event.event_type); const Icon = typeConfig.icon; return (<Card key={base44.entities.Event.id} className="bg-black/50 border-gray-800"><CardContent className="p-4"><div className="flex items-start justify-between mb-3"><div className="flex items-center gap-3"><div className={`p-2 rounded-lg ${typeConfig.color}`}><Icon className="w-4 h-4 text-white" /></div><div><h5 className="font-semibold text-white">{base44.entities.Event.title}</h5><p className="text-sm text-gray-400">{base44.entities.Event.event_category}</p></div></div><div className="flex gap-2"><Button variant="ghost" size="icon" onClick={() => handleEdit(event)}><Edit className="w-4 h-4" /></Button><Button variant="ghost" size="icon" onClick={() => handleDelete(base44.entities.Event.id)}><Trash2 className="w-4 h-4 text-red-400" /></Button></div></div>{base44.entities.Event.description && (<p className="text-sm text-gray-300 mb-3">{base44.entities.Event.description}</p>)}<div className="space-y-2 text-sm"><div className="flex items-center gap-2 text-gray-400"><Clock className="w-4 h-4" />{new Date(base44.entities.Event.start_date).toLocaleString()}</div>{base44.entities.Event.location && (<div className="flex items-center gap-2 text-gray-400"><MapPin className="w-4 h-4" />{base44.entities.Event.location}</div>)}<div className="flex items-center gap-2 text-gray-400"><Users className="w-4 h-4" />{base44.entities.Event.target_users.length > 0 ? `${base44.entities.Event.target_users.length} atletas` : 'Todos os atletas'}</div></div><div className="flex gap-2 mt-3">{base44.entities.Event.is_mandatory && (<Badge className="bg-red-600 text-white text-xs">Obrigatório</Badge>)}{base44.entities.Event.max_participants && (<Badge className="bg-blue-600 text-white text-xs">Máx. {base44.entities.Event.max_participants}</Badge>)}<Badge className={base44.entities.Event.is_active ? 'bg-green-600' : 'bg-gray-600'}>{base44.entities.Event.is_active ? 'Ativo' : 'Inativo'}</Badge></div></CardContent></Card>);})}</div></div>
      {pastEvents.length > 0 && (<div className="space-y-4"><h4 className="text-lg font-medium text-gray-400 flex items-center gap-2"><Clock className="w-5 h-5" />Eventos Passados ({pastEvents.length})</h4><div className="grid grid-cols-1 md:grid-cols-3 gap-4">{pastEvents.slice(0, 6).map(event => { const typeConfig = getEventTypeConfig(base44.entities.Event.event_type); const Icon = typeConfig.icon; return (<Card key={base44.entities.Event.id} className="bg-gray-900/30 border-gray-800 opacity-75"><CardContent className="p-4"><div className="flex items-center gap-3 mb-2"><div className={`p-2 rounded-lg ${typeConfig.color} opacity-60`}><Icon className="w-4 h-4 text-white" /></div><div><h5 className="font-medium text-gray-300">{base44.entities.Event.title}</h5><p className="text-xs text-gray-500">{new Date(base44.entities.Event.start_date).toLocaleDateString()}</p></div></div></CardContent></Card>);})}</div></div>)}
    </div>
  );
}
