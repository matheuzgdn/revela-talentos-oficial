...
const TasksTab = ({ lead }) => {
  ...
  const loadTasks = useCallback(async () => {
    const relatedTasks = await appClient.entities.CustomTask.filter({ related_lead_id: lead.id, related_lead_type: 'Lead' });
  ...
  const handleAddTask = async () => {
    ...
    await appClient.entities.CustomTask.create({ title: newTaskTitle, related_lead_id: lead.id, related_lead_type: 'Lead', status: 'pending' });
    setNewTaskTitle('');
    loadTasks();
  };
  ...
  const toggleTask = async (task) => {
    ...
    await appClient.entities.CustomTask.update(task.id, { status: newStatus });
    loadTasks();
  };
...
export default function LeadDetailsModal({ lead, isOpen, onClose, onUpdate }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await appClient.entities.CRMLead.update(id, updateData);
      onUpdate();
