...
export default function CheckoutSuccess({ platformUrlOverride }) {
  ...
  return (
    <div className="relative min-h-screen bg-black text-white font-sans overflow-hidden flex items-center justify-center">
      ...
      {/* Content Container */}
      <div className={`relative z-10 w-full max-w-5xl mx-auto px-6 py-12 transition-all duration-1000 transform ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
        
        {/* Header with Logos */}
        <div className="flex justify-between items-center mb-12">
          <img src={REVELA_LOGO} alt="Revela Talentos" className="h-16 md:h-20 drop-shadow-[0_0_15px_rgba(255,255,255,0.3)] transition-transform hover:scale-105 cursor-pointer" />
          <img src={EC10_LOGO} alt="EC10" className="h-12 md:h-16 drop-shadow-[0_0_15px_rgba(255,255,255,0.3)] transition-transform hover:scale-105 cursor-pointer" />
        </div>
            ...
            <div className="bg-black/40 backdrop-blur-md rounded-2xl p-8 border border-white/5 shadow-inner">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <p className="text-gray-500 text-sm mb-1 flex items-center gap-1"><User className="w-4 h-4"/> Atleta</p>
                    <p className="text-white font-medium truncate">{clientInfo.name}</p>
                  </div>
                  <div>
                    <p className="text-gray-500 text-sm mb-1 flex items-center gap-1"><Mail className="w-4 h-4"/> Email</p>
                    <p className="text-white font-medium truncate" title={clientInfo.email}>
                      {clientInfo.email.length > 20 ? clientInfo.email.substring(0, 20) + '...' : clientInfo.email}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div>
                    <p className="text-gray-500 text-sm mb-1 flex items-center gap-1"><CreditCard className="w-4 h-4"/> Valor Total</p>
