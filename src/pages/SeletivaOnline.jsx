  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [seletivaEvents, applications] = await Promise.all([
        base44.entities.SeletivaEvent.filter({ is_published: true, status: 'open' }, '-created_date'),
        currentUser ? base44.entities.SeletivaApplication.filter({ user_id: currentUser.id }, '-created_date') : Promise.resolve([])
      ]);
      ...
      setUser(null);
      const seletivaEvents = await base44.entities.SeletivaEvent.filter({ is_published: true, status: 'open' }, '-created_date');
      setEvents(seletivaEvents);
    }
    setIsLoading(false);
  };
